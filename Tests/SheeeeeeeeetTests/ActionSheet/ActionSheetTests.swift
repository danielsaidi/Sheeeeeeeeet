    ...
    override func spec() {
        ...
        describe("setup items") {
            ...
            it("reloads data an extra time when setting up items") {
                let preExecs = sheet.recorder.invokations(of: sheet.reloadData)
                expect(preExecs.count).to(equal(1))
                sheet.setup(items: [])
                let postExecs = sheet.recorder.invokations(of: sheet.reloadData)
        ...
        describe("loading view") {
            ...
            it("sets up action sheet") {
                let execs = sheet.recorder.invokations(of: sheet.setup as MockActionSheet.SetupSignature)
        ...
        describe("laying out subviews") {
            ...
            it("refreshes sheet") {
                ...
                let execs = sheet.recorder.invokations(of: sheet.refresh)
        ...
        context("presentation") {
            ...
            describe("when dismissed") {
                ...
                it("it calls presenter") {
                    ...
                    let execs = presenter.invokations(of: presenter.dismiss)
                    execs[0].arguments()
                    expect(counter).to(equal(1))
                    expect(execs.count).to(equal(1))
                }
            ...
            describe("when presented from view") {
                ...
                it("calls presenter") {
                    ...
                    let execs = presenter.invokations(of: presenter.present as MockActionSheetPresenter.PresentFromViewSignature)
                    expect(execs.count).to(equal(1))
                    let exec = execs[0]
                    exec.arguments.3()
            ...
            describe("when presented from bar button item") {
                ...
                it("calls presenter") {
                    ...
                    let execs = presenter.invokations(of: presenter.present as MockActionSheetPresenter.PresentFromItemSignature)
                    expect(execs.count).to(equal(1))
                    let exec = execs[0]
                    exec.arguments.3()
                    expect(exec.arguments.0).to(be(sheet))
        ...
        describe("refreshing") {
            ...
            it("refreshes all components") {
                let headerExecs = sheet.recorder.invokations(of: sheet.refreshHeader)
                let headerVisibilityExecs = sheet.recorder.invokations(of: sheet.refreshHeaderVisibility)
                let itemsExecs = sheet.recorder.invokations(of: sheet.refreshItems)
                let buttonsExecs = sheet.recorder.invokations(of: sheet.refreshButtons)
                let refreshExecs = presenter.invokations(of: presenter.refreshActionSheet)
        ...
        describe("handling tap on item") {
            ...
            it("calls select action without dismiss if item has none tap action") {
                ...
                let execs = sheet.recorder.invokations(of: sheet.dismiss)
